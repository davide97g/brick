...
Everything Brick knows lives in one JSON file inside an App Group container on your device,
readable only by Brick and its two extensions:
...
names or bundle identifiers. Brick stores those bytes and never decodes them. The app is
structurally incapable of knowing which apps you blocked, and no code path exists that could
report them.

## Screen Time access

Brick asks for Family Controls authorization for the individual case: your device restricting
...
Brick reads the factory UID of your tag to recognise your brick, and writes an identifier onto
the tag when you pair it. No other tag data is read or retained.

## Children

Brick is not directed at children and collects no data from anyone.
